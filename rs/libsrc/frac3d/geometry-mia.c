...
/*                                                                           */
/* Modified:  Mia Suess (Mia)                                                */
/*            Institut fuer Wasserbau                                        */
/*            Universitaet Stuttgart                                         */
/*            email: mia.suess@iws.uni-stuttgart.de                          */
/*            Aim: Generation of 1D traces in 2D domain (22.07.2002)         */
/*                                                                           */
/* Functions: void gen_random_fracture_list()                                */
/*            void gen_random_trace_list()                                   */
...
/*****************************************************************************/
/*  Geometrie-Eigenschaften der random Kluefte generieren (3D)               */
...
/*****************************************************************************/
/*  Geometrie-Eigenschaften der random Kluefte generieren (2D)               */
/*  Funktion:  gen_random_trace_list                                         */
/*****************************************************************************/

void gen_random_trace_list() /* call by function gen_fracture_list in
				   prog_functions.c */
{
   int i,j;
   int help_j;
   ...
   i                = ntrace_det;
   trace_length     = trace_length_det;      /* assign the deterministic  */
                                             /* trace length to the total */
                                             /* length                    */ 
   trace_dens_2d_sim = 0.0; 

   while (trace_dens_2d_sim < trace_dens_2d) 
   {
      /* generate mid-point 'midpt' of the new trace*/
      midpt.x = dom_min.x + ran3(&rseed) * (dom_max.x - dom_min.x) ;  
      midpt.y = dom_min.y + ran3(&rseed) * (dom_max.y - dom_min.y) ;
      midpt.z = 0.00 ;

      /***********************************************************************/
      /* Im ersten Stochastik -- Durchgang (Bed: i==ntrace_det) werden die    */
      /*    Variablen eingelesen                                             */
      /*                                                                     */
      /*                                                                     */
      /***********************************************************************/
      if (i == ntrace_det)
      {
         ...
         if ((percentage_orient == NULL) || (alpha == NULL) || 
          (Phi == NULL) || (kappa == NULL))
         {
            printf("\nMemory allocation failed  (trace orientation) \n");
            exit (-1);
         }
         ...
         for (j=0; j<norient; j++)
         {
            ...
            percentage_orient[j] = get_var_double(uvar, char_percentage_orient);
            percentage_orient_sim[j] = 0.0;  /*set default value*/
            /*TODO Azimut ->Streichwinkel; Phi in Headerfile*/
            alpha[j]                 = get_var_double(uvar, char_alpha);
            Phi[j]                   = 90.0;
            kappa[j]                 = get_var_double(uvar, char_kappa);
      ...
      /***********************************************************************/
      /* Normalenvektor der Linie generieren  norm                           */
      /*   1. Anwendung der spaehrischen Normalverteilung                    */
      /*   2. Winkel phi : Winkel Normalenvektor und seiner Projektion       */
      /*      in der xy-Ebene berechnen                                      */
      /*                                                                     */
      ...
      /*                            horizont. + vertikale Spurl.vert.        */
      /***********************************************************************/
      if (i == ntrace_det) length_dist_type = get_var_integer(uvar,"length_dist_type");
      ...
      switch(length_dist_type)
      {
         case 1:    /* Spurlaengenverteilung mit Erlang 2 Verteilung */
            if (i == ntrace_det)
            {
               ...
               X0_v            = get_var_double(uvar, "X0_v");
               epsilon_Erlang  = get_var_double(uvar, "epsilon_Erlang");
               fprintf(stdout,"\n   lambda_h=%.2f \t X0_h=%.2f \t epsilon_Erlang=%.2f", 
                                    lambda_h, X0_h, epsilon_Erlang);
               /*fprintf(stdout,"\n   lambda_v=%.2f \t X0_v=%.2f \t epsilon_Erlang=%.2f", 
                                    lambda_v, X0_v, epsilon_Erlang);*/
            }
            /*Not used because all fractures strictly vertical*/  
            if (phi >= 0.00 && phi <= pi/4) 
            {
               xrand = ran3(&rseed);
               length[0] = erl2vert(xrand, lambda_h, X0_h, epsilon_Erlang);
            ...
            /*Only length[0] important since only intersection with x,y-plane needed*/
            else if ( phi > pi/4  &&  phi <= pi/2) 
            {
               xrand = ran3(&rseed);
               length[0] = erl2vert(xrand, lambda_h, X0_h, epsilon_Erlang);
      ...
      /***********************************************************************/
      /* generate trace edge points:  pt[0] / pt[1]                          */
      /*                                                                     */
      /***********************************************************************/
      pt[0].x = midpt.x + 0.5 * (-lambda1*vector_r.x - lambda2*vector_b.x);
      pt[0].y = midpt.y + 0.5 * (-lambda1*vector_r.y - lambda2*vector_b.y);
...
***************/
      ...
      /**********************************************************************/
      /* Calculating trace density 'trace_dens_2d_sim' of the generated     */
      /* field.                                                             */
      /*                                                                    */
      /* Case 1: frac_dens_type = 1  --> trace_dens_2d_sim [m / m^2]        */
      /*         Calculating the trace length of the trace 'i' and          */
      /*         add it to the total trace length 'trace_length'            */
      /*                                                                    */
      /* Case 2: frac_dens_type = 2  --> trace_dens_2d_sim [trace / m^2]    */
      /**********************************************************************/
      frac_dens_type = get_var_integer(uvar, "frac_dens_type");
      switch (frac_dens_type) {
      case 1:  trace_length += length[0];
               trace_dens_2d_sim = trace_length / domain_area; 
               break;
      case 2:  trace_dens_2d_sim = ntrace / domain_area; 
               break;
      }
   ...
   switch (frac_dens_type) {
   case 1:  fprintf(stdout,"\n\nTrace density [m/m^2] trace_dens_2d_sim=%f ", 
                             trace_dens_2d_sim);
            break;
   case 2:  fprintf(stdout,"\n\nTrace density [trace/m^2] trace_dens_2d_sim=%f ", 
                              trace_dens_2d_sim);
            break;
   }
   fprintf(stdout,"\nNumber of trace elements ntrace=%d \n", ntrace);

   fprintf(stdout,"\nHauptorientierungen  ");
   for (i = 0; i < norient; i++)
   {
      if (i==0) fprintf(stdout,"[%d] = %7.3f [\%]\n", 
                       i,percentage_orient_sim[i]/ntrace*100);
      else      fprintf(stdout,"                     [%d] = %7.3f [\%]\n", 
                       i,percentage_orient_sim[i]/ntrace*100);
   }
   ...
   /**************************************************************************/
   /* Speicher wieder frei geben                                             */
   /*  (frueher nicht moeglich, da innerhalb der (ntrace) -- for-Schleife     */
   /*   darauf zurueckgegriffen wird)                                        */
   /**************************************************************************/
   free(percentage_orient); 
   free(alpha); 
