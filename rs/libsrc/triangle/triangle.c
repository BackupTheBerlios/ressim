...
/*   minarea: minimum area for refinement, specified after -m switch.        */
/*   usertest: -u switch.                                                    */
...
void syntax()
{
...
#else /* not CDT_ONLY */
#ifdef REDUCED
  printf("triangle [-prq__a__m__uAcDjevngBPNEIOXzo_YS__lQVh] input_file\n");
#else /* not REDUCED */
  printf("triangle [-prq__a__m__uAcDjevngBPNEIOXzo_YS__iFlsCQVh] input_file\n");
#endif /* not REDUCED */
#endif /* not CDT_ONLY */
  ...
  printf("    -m  Applies a minimum triangle area constraint.\n");  
...
"        without a number following.  Each area specified may include a\n");
  ...
  printf(
"    -m  Imposes a minimum triangle constraint on refinement. No triangles\n");
  printf(
"        that are smaller than the given size will be refined further. Some\n");
  printf(
"        Finite Volume Methods do not like too large difference between the\n");
  printf(
"        size of the elements. Note that there will still be some triangles\n");
  printf(
"        that are smaller than this size; it is just that they won't be split\n");
  printf(
"        any more.");
  printf(
			...
			if (((argv[i][j + 1] >= '0') && (argv[i][j + 1] <= '9')) ||
				(argv[i][j + 1] == '.')) {
					...
					while (((argv[i][j + 1] >= '0') && (argv[i][j + 1] <= '9')) ||
						...
						if (b->minarea < 0.0) {
							printf("Error:  Minimum area must be greater than or equal to zero.\n");
							triexit(1);
						}
				}
		}
...

{
  ...
  /* Don't refine to elements smaller than the allowed minimum size. If we  */
  /* insert a vertex in the middle of the element, we'll split the triangle */
  /* into three new triangles. If we had split it at the centroid, then it  */
  /* would be sufficient to be at least three times as large as the desired */
  /* minimum size, since all child triangles would be of equal size (todo:  */
  /* prove the above conjecture -- a simulation confirms it empirically).   */
  /* However, splittriangle() inserts a new vertex at the circumcenter      */
  /* which means that three times is the upper bound, i.e. a necessary but  */
  /* unfortunately not sufficient condition.                                */
  if (area < 3 * b->minarea)
  	return;
