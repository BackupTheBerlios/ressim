               ...
               int nplv, int hiddenflag)
{
  ...
  double v[3];
  int incr, ic1, ic2, linecount;
  float colr, colg, colb, params[4], colr2, colg2, colb2, texcol;
  float pt_size;
  double v1[3], fac, speed, maxspd, minspd, xv, yv, zv, uv, vv, wv, val;
  double a[4][3], xn, yn, zn, t, x1, y1, z1, vlogoffset;
  double x2, y2, z2, x3, y3, z3, theta, xa, ya, d; 
  double d90 = 1.570796327;
  long ival, ival2;
  char str[30];
  int nregpts, ntbig, nnums, nlsolid, nlsmooth, nthist;
  static int nregpts0, ntbig0, nnums0, nlsolid0, nlsmooth0, nthist0;
  static int nregpts1, ntbig1, nnums1, nlsolid1, nlsmooth1, nthist1;
  void getrgbcol(int ic1, float *colr, float *colg, float *colb);
  void p_s_header(), p_s_doit();
   ...
   if (drawnew3dflag && hiddenflag == 0)
     {
      nregpts0 = 0;  ntbig0 = 0;  nnums0 = 0;  nlsolid0 = 0;  nlsmooth0 = 0;
      nthist0 = 0;
      for (i = 0; i < nplv; i++)
        {
         itype = drawplv[i].itype;
         if (itype == PONLY || itype == PANDNUM || itype == TPOINT) nregpts0++;
         if (itype == TBIG || itype == TSIZE) ntbig0++;
   ...
   if (drawnew3dflag && hiddenflag == 1)
     {
      nregpts1 = 0;  ntbig1 = 0;  nnums1 = 0;  nlsolid1 = 0;  nlsmooth1 = 0;
      nthist1 = 0;
      for (i = 0; i < nplv; i++)
        {
         itype = drawplv[i].itype;
         if (itype == PONLY || itype == PANDNUM || itype == TPOINT) nregpts1++;
         if (itype == TBIG || itype == TSIZE) ntbig1++;
   ...
   if (hiddenflag == 0)
     {
      nregpts = nregpts0;  ntbig = ntbig0;  nnums = nnums0;  
      nlsolid = nlsolid0;  nlsmooth = nlsmooth0;  nthist = nthist0;
     }
   if (hiddenflag == 1)
     {
      nregpts = nregpts1;  ntbig = ntbig1;  nnums = nnums1;  
   ...
   /*  Draw large tracer points.  */
   if (ntbig > 0)
     {
      for (i = 0; i < nplv; i += incr)
        {
         itype = drawplv[i].itype;
         if (itype == TBIG || itype == TSIZE)
           {
            ...
            params[2] = v[2];
            if (itype == TBIG)
               params[3] = span/(magnify*150.);
            if (itype == TSIZE)
               params[3] = drawplv[i].y2;
            icosaflat(params);
           }
        }
     }
