...
public enum Corner {
    ...
    X x() {
        return X.values()[(this.ordinal() >> 0) & 1];
    }

    Y y() {
        return Y.values()[(this.ordinal() >> 1) & 1];
    }

    Z z() {
        return Z.values()[(this.ordinal() >> 2) & 1];
    }

    // compose a corner given the direction (from the center) in each of the
    // various dimensions
    static Corner xyz(X x, Y y, Z z) {
        int ordinal = (((z.ordinal()) * Y.values().length + y.ordinal())
     ...
     * 
     * @param directions
     *            Array containing a direction (from the center) for each of the
     *            axis. There must be one and only one direction for each
     *            dimension (i.e. 3)
     * @return The corner located from the direction vectors (relative to
     *         center)
     */
     ...
     * 
     * @return New corner whose ordinal will be in the range [0..3], based on
     *         the location of this corner.
     */
    @SuppressWarnings("unused")
    ...
    public enum X {
        ...
        static X from(Direction direction) {
            return X.values()[direction.ordinal()];
        }
    }
    ...
    public enum Y {
        ...
        static Y from(Direction direction) {
            return Y.values()[direction.ordinal()];
        }
    }
    ...
    public enum Z {
        ...
        static Z from(Direction direction) {
            return Z.values()[direction.ordinal()];
        }
    }
