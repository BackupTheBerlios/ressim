...
public abstract class Delaunay2D<EdgeData> {
    ...
    /**
     * Mini-algebra for vertices. Accessor methods to save us typing the
     * indexing operation; this allows us to treat integers as placeholders for
     * point references, and thus operate on a homomorphic image of the set
     * instead of reporting true references. These are made available for
     * subclasses so that they don't need to maintain their own reference to the
     * point selection.
     */
    ...
    /**
     * Mini-algebra for edges. encode a directed edge between two points as a
     * double-word value accessor functions that allows us to get the individual
     * vertex endpoints out of a value representing an edge.
     */
     ...
     * 
     * The first item in the pair is the auxiliary data returned from the client
     * that is associated with the edge, the second is the index of the mate of
     * this edge.
     */
    private final Map<Long, Pair<EdgeData, Integer>> values = new HashMap<Long, Pair<EdgeData, Integer>>();
     ...
     * time. Vertices that are not yet discovered, have a default count of zero.
     * As they are put on the frontier, they are being reference counted. When
     * they are taken off the frontier (and thus behind it), they are given the
     * value below to indicate so.
     */
    ...
    private EdgeData birth(int a, int b, boolean initial) {
        ...
        if (!boundary) {
            ...
            // that the indices in the usage array is different from that used
            // to
    ...
    /**
     * E.g. to find the area of a polygon represented by the cloud p: <code>
     *  double area = 0.;
     ...
     * 
     * @param p
     *            (Super)set of vertices that should be triangulated. The
     *            contents of this array should not be changed in the callbacks.
     * @param index
     *            Index of the first element that should be considered part of
     *            the subset to be triangulated.
     * @param length
     *            Number of points in the subset, continuously from the first
     *            point.
     */
     ...
     * 
     * @return Point which is at one extreme of the vertex set, i.e. there is no
     ...
     * @param relation
     *            If false, the function acts as a classifier where the points
     *            are ranked after at which side they are of the half-plane
     *            spanned by the (unbounded) line between a and b. This is the
     *            default. If true, the function acts as a relation that ranks
     *            points b and c based on which is most to the left and below a.
     *            I.e. points that are on the extension of the line but outside
     *            of the segment itself are considered to be on the "right" (not
     *            less-or-equal). This distinction has been made to enable the
     *            algorithm to pick out the smallest initial edge while still
     *            not erraneously accept three points on a row when finding
     *            triangles.
     * @return True if point C is to the right of (or not on, if the truly flag
     *         is not set) the line AB, when viewed along the line from its
     *         origin A.
     */
    protected int side(int a, int b, int c) {
        return side(a, b, c, false);
    }
     ...
     * 
     * @return An edge that is part of the triangulation and that can serve as
     *         the initial frontier. The frontier will always be to the right of
     *         this edge.
     */
     ...
     * @param a
     * @param b
     *            Endpoints for the edge that is part of the triangulation's
     *            frontier.
     * @param c
     *            Point that is currently outside of the triangulation but is a
     *            candidate to be added.
     * @return Squared radius in the circumcircle between the three points
     *         formed by the circumcircle. These values can be compared to find
     *         the closest of two candidate points.
     */
     ...
     * @param c
     *            Point that is currently outside of the triangulation but is a
     *            candidate to be added. Must be to the right of the edge
     *            between a and b.
     * @return Length along the line normal to the edge between a and b, to get
     *         from the edge to the point c. If the point c is to the left of
     *         the directed line starting at a and ending at b, the length will
     *         be negative.
     */
     ...
     * @return Point that together with the edge forms a new piece of the
     *         triangulation.
     */
            ...
            if (side(a, b, c, false) == RIGHT) {
                ...
                if (visible(a, c) && visible(b, c)) {
                    ...
                    if (candDist < bestDist
                            || (Math.abs(candDist - bestDist) < Tolerances.smallEps && candUse > bestUse)) {
                        bestDist = candDist;
                        bestUse = candUse;
                        mate = c;
                    }
     ...
     */
    @SuppressWarnings("unused")
    protected EdgeData forEachEdge(int a, int b) {
        return null;
    }
    ...
    /**
     * Callback that is used when an interior edge is encountered for the second
     * time. The points are so that the directed edge from a to b has the next
     * triangle to be created to the right (i.e. opposite order of the first
     * call to forEachInterior for this edge).
     * 
    ...
    public static class Tests {
        ...
        public void simple() {
            ...
            // triangles
            final int[] expectedTris = new int[] { 0, 1, 4, 0, 4, 6, 0, 6, 3,
                    3, 6, 2, 2, 6, 5, 2, 5, 1, 1, 5, 4, 4, 5, 6 };
            final int[] actualTris = new int[expectedTris.length];

            // boundary edges
            final int[] expectedBnds = new int[] { 0, 1, 3, 0, 2, 3, 1, 2 };
            final int[] actualBnds = new int[expectedBnds.length];

            // interior edges
            final int[] expectedInts = new int[] { 1, 4, 4, 0, 0, 4, 4, 6, 6,
                    0, 0, 6, 6, 3, 3, 6, 6, 2, 2, 6, 6, 5, 5, 2, 2, 5, 5, 1, 1,
                    5, 5, 4, 4, 1, 4, 5, 5, 6, 6, 4, };
        ...
        public void cocircular() {
            final Point3D[] p = new Point3D[] { ll, lm, lu, cl, cm, cu, rl, rm,
                    ru };

            final int[] expectedTris = new int[] { 0, 1, 3, 3, 1, 4, 3, 4, 6,
                    4, 1, 2, 4, 2, 5, 4, 5, 7, 4, 7, 6, 7, 5, 8 };
            final int[] actualTris = new int[expectedTris.length];
